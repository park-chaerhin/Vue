  ...
  <v-app>
    ...
    <v-card>
      ...
      <v-main>
        <v-container>
          <v-row my-5>
            <v-col cols="8" offset="1"> <!--글쓰기-->
              <!--실행되자 마자 입력포커스를 가지도록 autofocus 설정-->
              <v-text-field label="할 일" autofocus v-model="sTodoTitle">
              </v-text-field>
            </v-col>
            <v-col cols="2" my-2> <!--입력된 글 출력-->
              <v-btn fab max-height="50px" max-width="50px" color="orange" dark @click="fnSubmitTodo()">
                <v-icon>add</v-icon>
              </v-btn>
            </v-col>
          </v-row>
          <v-row>
            <v-col cols="12"> <!--12만큼 글씨영역 -> 내용 길어지면 자동 말줄임표-->
              <v-list two-line v-for="item in oTodos" :key="item.key"> <!--v-for + key 필수-->

                <!--item.b_edit값을 통해 읽기모드인 경우만 표시-->
                <v-card flat color="grey lighten-3" v-if="!item.b_edit"> <!--b_edit:수정모드true/읽기모드false-->
                  ...
                  <v-list-item class="py-2">
                    ...
                    <v-list-item-content>
                      ...
                      <v-list-item-subtitle class="mt-2">
                        <!--수정 아이콘 표시하고 클릭하면 수정 모드로 변경 / 선택한 값만 수정모드=item['.key'], 이거 없으면 전체리스트선택-->
                        <v-icon class="pointer" @click="fnSetEditTodo(item['.key'])">edit</v-icon>
                ...
                <v-card v-else dark>
                  <v-list-item class="py-2">
                    ...
                    <!--v-list-item안에서 텍스트 입력과 버튼 사용을 위해 v-card 엘리먼트 사용-->
                    <v-card-text>
...
export default {
  ...
  // 파이어베이스를 쉽게 '사용'하도록 oTodos 변수로 변경
  firebase:{
    oTodos: oTodosinDB
  },
  methods:{
    ...
    fnSaveEdit(pItem){
      ...
      oTodosinDB.child(sKey).set({ ///set: 바꿔줘!
