...
<template>
    <v-container>
        ...
        <v-row>
            <v-col offset="2" cols="8">
                <!--3줄로 편집 제한-->
                <v-text-field name="info" label="사진설명" v-model="sInfo" multi-line rows="3"></v-text-field>
            </v-col>
        </v-row>
        <v-row>
            <v-col cols="12" class="text-center">
                <v-btn color="orange" dark @click="fnSubmitPost()">
...
export default{
    ...
    data(){
        return{
            ...
            aPictures: oPictureData.aPictures, //초기값으로 JSON 파일의 이미지 배열 저장
            sTitle: '',
            sInfo: '',
            sPicUrl: ''
        }
    ...
    mounted(){
        ...
        let nIndex = Math.floor(Math.random()*12) //Math = floor(내림), ceil(올림)
    ...
    methods:{
        fnSubmitPost(){
            //DB에 저장하고 홈화면으로 이동 : 페이지바꾸기=redirection
            oPicturesinDB.push({
                "url": this.sPicUrl,
                "title": this.sTitle,
                "info": this.sInfo
